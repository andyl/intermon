defmodule Util.IoStreamError do
  ...
  def exception(opts) do
    ...
    %Util.IoStreamError{message: "error during streaming: #{formatted}", reason: reason}
  end
end

defmodule Util.IoStream do
  @moduledoc """
  Defines an `Util.IoStream` struct returned by `IO.stream/2` and `IO.binstream/2`.
  ...
  def __build__(device, raw, line_or_bytes) do
    %Util.IoStream{device: device, raw: raw, line_or_bytes: line_or_bytes}
  end
